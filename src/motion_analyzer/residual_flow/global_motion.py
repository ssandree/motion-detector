...
class MotionCompensationLayers:
    """Per frame-pair motion maps used for residual flow and debug overlays."""
    ...
    metadata: GlobalMotionResult = field(default_factory=lambda: GlobalMotionResult(
        dx=0.0,
        dy=0.0,
        scale=1.0,
        rotation=0.0,
...
def _flow_magnitude_map(flow: np.ndarray, blur_ksize: int = 7, blur_sigma: float = 1.5) -> np.ndarray:
    from motion_analyzer.residual_flow.pixel_motion import _normalize_map
...
def compute_motion_compensation_layers(
    ...
    *,
    ...
    """
    from motion_analyzer.residual_flow.pixel_motion import (
        _normalize_map,
        compute_frame_diff,
...
def compensate_global_motion(
    ...
    *,
    ...
    """Return residual motion map + original flow + metadata."""
    ...
    return layers.residual_map, layers.original_flow, layers.metadata
