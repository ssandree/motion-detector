...
class MotionConfig:
    ...
    use_jitter_suppression: bool = True
...
def compute_motion_map_pair(
    ...
    if config.use_global_compensation:
        from motion_analyzer.v1.global_motion import compute_motion_compensation_layers
        ...
        global_meta = {
            ...
            "background_jitter_flag": gm.background_jitter_flag,
            "background_jitter_score": gm.background_jitter_score,
            "jitter_suppression_applied": gm.jitter_suppression_applied,
        }
        frame_diff_mean = float(layers.suppressed_map.mean())
        motion_map = layers.suppressed_map if config.use_jitter_suppression else layers.residual_map
    ...
    for frame_idx, timestamp, frame in iter_sampled_frames(video_path, config.target_fps):
        ...
        if prev_frame is not None:
            ...
            if layers is not None:
                ...
                layer_maps["suppressed"].append(layers.suppressed_map)
